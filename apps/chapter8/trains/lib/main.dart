...
class NetworkImage extends StatefulWidget {
  NetworkImage({ this.src, this.fit });

  final String src;
  final ImageFit fit;

  @override
  NetworkImageState createState() => new NetworkImageState();
}
...
class NetworkImageState extends State<NetworkImage> {
  ...
  @override
  void didUpdateConfig(NetworkImage oldConfig) {
    if (oldConfig.src != config.src)
      loadImage();
  }

  void loadImage() {
    imageCache.load(config.src)..first.then(
      (ImageInfo imageInfo) {
        setState(() {
